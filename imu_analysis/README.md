...
---
# Análisis IMU: Evaluación de Pronación-Supinación

Este módulo presenta una metodología original desarrollada para el análisis cuantitativo de los síntomas motores de la Enfermedad de Parkinson (EP) utilizando datos de Unidades de Medición Inercial (IMU) (acelerómetro y giroscopio).

A diferencia de finger tapping esta prueba cuenta unicamente con un pipeline que ha sido diseñado y desarrollado íntegramente como parte de esta investigación, centrándose en la discriminabilidad estadística entre estadios de la enfermedad.
...
- **Puntuaciones 1–4**: 3 sujetos simulando niveles de EP (30 pruebas por puntuación).
- **Sensores**: Acelerómetro de 3 ejes y Giroscopio de 3 ejes.
- **Tarea**: Ítem 3.6 de la escala MDS-UPDRS (Pronación-Supinación).
...
- `PRONATION_SUPINATION/`: Datos brutos en formato JSON, organizados por carpetas (Score 0 a Score 4).
- `resultados_pronosupina_completo/`: Figuras generadas, tablas y el modelo LDA entrenado.
- `pronosupina_analisis_completo.ipynb`: El cuaderno (notebook) principal de orquestación.
