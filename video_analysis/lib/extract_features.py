...
def extract_tsfresh_features(dataset_identifier):
    ...
    #TSFresh invocation for capturing all features (NO select_features here to avoid data leakage)
    extracted_features = extract_features(data_final, column_id="ID", impute_function=impute)
    
    extracted_features.to_csv(config.output_files_dir+dataset_identifier+"_tsfresh_features.csv")
...
def extract_classical_features(dataset_identifier):
    ...
    # ITI Features (Inter-Tap Interval)
    iti_features = data_final.groupby('ID').apply(
        lambda g: calculate_iti_features(g, fps=fps_per_video.at[g.name, 'FPS'])
    )
    iti_features.columns = ['ITI_MEAN', 'ITI_STD', 'ITI_CV']
    iti_features = iti_features.reset_index()
    summary_df = summary_df.merge(iti_features, on='ID', how='left')

    # Jerk RMS
    jerk_rms = data_final.groupby('ID').apply(
        lambda g: calculate_jerk_rms(g, fps=fps_per_video.at[g.name, 'FPS'])
    )
    summary_df['JERK_RMS'] = jerk_rms.values
    ...
    fft_features = data_final.groupby('ID').apply(
        lambda g: calculate_fft_features(g, fps=fps_per_video.at[g.name, 'FPS'])
    )
...
def extract_fi_tsfresh_features(dataset_identifier):
    ...
    # Extract features (NO select_features here to avoid data leakage)
    extracted_features = extract_features(series_df, column_id="ID", impute_function=impute)
    extracted_features.to_csv(config.output_files_dir+dataset_identifier+"_fi_tsfresh_features.csv")
...
def calculate_fft(group, column, fps=30):
    """
    Calculate the Fast Fourier Transform (FFT) of a signal column in Hz.
    ...
    Returns:
        pandas.Series: A series containing two values:
            [0] dominant_freq (float): The dominant frequency component of the signal (in Hz).
    ...
    signal_data = group[column].values
    n = len(signal_data)
...
def calculate_fft_features(group, fps=30):
    """
    ...
    """      
    # FFT para AMPLITUDE
    amplitude_dominant_freq, amplitude_power = calculate_fft(group, 'SMOOTHED_AMPLITUDE', fps)
    
    # FFT para VELOCITY
    velocity_dominant_freq, velocity_power = calculate_fft(group, 'VELOCITY', fps)
    
    return pd.Series([amplitude_dominant_freq, amplitude_power, velocity_dominant_freq, velocity_power])

def calculate_iti_features(group, fps):
    """
    Calculate the Inter-Tap Interval (ITI) features: mean, std, and coefficient of variation.
    ...
    Returns:
        pandas.Series: A series containing ITI_MEAN, ITI_STD, and ITI_CV.
    """
    amplitude = group['SMOOTHED_AMPLITUDE'].values
    peaks, _ = find_peaks(amplitude, distance=fps//3)
    if len(peaks) < 2:
    ...
    iti = np.diff(peak_times)
    return pd.Series([np.mean(iti), np.std(iti), np.std(iti)/np.mean(iti) if np.mean(iti) > 0 else np.nan])

def calculate_jerk_rms(group, fps):
    """
    Calculate the Root Mean Square (RMS) of the Jerk (derivative of acceleration).
    ...
    Returns:
        float: RMS value of the jerk signal.
    """
    acc = group['ACCELERATION'].values
    jerk = np.diff(acc) / (1/fps)
    return np.sqrt(np.mean(jerk**2)) if len(jerk) > 0 else np.nan
